...
def native_gqa_sinks(
    ...
    for i in range(len(context_lens)):
        ...
        if is_extend:
            ...
            prefix_len = context_lens[i].item() - extend_len
            q = query[last : last + extend_len]
            last += extend_len
        else:
            prefix_len = context_lens[i].item() - 1
            q = query[last : last + 1]
            last += 1
    ...
    def forward_extend(
        ...
        if not self.use_mla:
            ...
            if sinks is not None:
                ...
                #     extend_seq_lens=self.forward_metadata.extend_seq_lens_cpu_int,
                #     scale=layer.scaling,
